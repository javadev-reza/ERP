...
import static com.microservice.erp2017.constant.WebConstant.MessageResource.LABEL_UNAUTHORIZED;
 ...
 */
@RestController
@RequestMapping("/auth-super-user")
public class AuthSuperUserController extends RestUtil {
    
    @Autowired AuthService authService;
    
    @ApiOperation("Api For Sign-In") 
    @PostMapping("/sign-in")
    public ResponseEntity signIn(@Valid @RequestBody VO_T_Auth vo, HttpServletResponse httpResponse, HttpServletRequest httpRequest) {
        try{
            if(CommonUtil.isNullOrEmpty(vo.getUserName()) || CommonUtil.isNullOrEmpty(vo.getPassword())) {
                ...
                Map<String, Object> resultData = authService.signinSuperUser(vo, httpRequest);
                if(CommonUtil.isNullOrEmpty(resultData)){
                    ...
                    if(CommonUtil.isNotNullOrEmpty(isValidToken)){
                        ...
                        setMessage(MessageInfo.WARNING_MESSAGE, LABEL_UNAUTHORIZED);
                        setResponse(getJsonResponse(HttpStatus.UNAUTHORIZED, getMessage()));
                    }
                }
            }
        }catch(Exception e){
    ...
    public ResponseEntity signUp(@Valid @RequestBody VO_T_Auth vo, HttpServletResponse httpResponse) {
        try{
            Map<String, Object> resultMap = authService.signupSuperUser(vo);
